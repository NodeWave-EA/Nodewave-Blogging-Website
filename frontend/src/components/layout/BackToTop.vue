<template>
  <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-8 scale-75"
    enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-200 ease-in"
    leave-from-class="opacity-100 translate-y-0 scale-100" leave-to-class="opacity-0 translate-y-8 scale-75">
    <div v-if="isVisible" class="fixed bottom-6 right-6 z-50 group">
      <!-- Circular Progress Background -->
      <div class="relative">
        <!-- Progress Circle -->
        <svg class="w-14 h-14 -rotate-90 transition-transform duration-300 group-hover:scale-110" viewBox="0 0 56 56">
          <!-- Background circle -->
          <circle cx="28" cy="28" r="24" fill="none" stroke="rgb(var(--color-border))" stroke-width="2" opacity="0.3" />
          <!-- Progress circle -->
          <circle cx="28" cy="28" r="24" fill="none" stroke="rgb(var(--color-primary))" stroke-width="2"
        ...
        <button @click="scrollToTop"
          ...
          aria-label="Back to top" type="button">
          <!-- Arrow Icon -->
          <svg class="w-5 h-5 mx-auto transition-transform duration-300 group-hover:scale-110" fill="none"
