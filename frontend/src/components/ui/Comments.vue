  ...
  <div class="comments-section">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center space-x-2 text-sm">
        ...
        <select v-model="sortBy" @change="sortComments"
    ...
    <div v-if="sortedComments.length > 0" class="space-y-6">
      <CommentItem v-for="comment in sortedComments" :key="comment.id" :comment="comment" :allow-replies="allowComments"
        :max-depth="maxDepth" :current-depth="0" @reply="handleReply" @like="handleLike" @report="handleReport"
        @delete="handleDelete" />

      <!-- Load More Button -->
      <div v-if="hasMoreComments" class="text-center">
        <button @click="loadMoreComments" :disabled="loadingMore"
    ...
    <div v-else-if="!loading" class="text-center py-12">
      ...
      <button v-if="allowComments" @click="focusCommentForm"
...
<script setup lang="ts">
  ...
  const handleReport = async (data: { commentId: number; reason: string }) => {
    ...
    try {
      await apiService.post(`/comments/${commentId}/report`, {
        data: { reason },
      })
