    ...
    <div class="relative z-10 container mx-auto px-4 text-center">
      <div class="max-w-4xl mx-auto">
        ...
        <div :class="['mb-4', badgeWrapperAnim]" data-aos="fade-down">
          <span
            :class="['inline-flex items-center rounded-full bg-blue-100/80 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 font-semibold backdrop-blur-sm border border-blue-200/50 dark:border-blue-800/50', badgePadding, badgeText]">
        ...
        <!-- Description -->
        <p v-if="description"
          :class="[descSize, 'text-slate-600 dark:text-slate-300 max-w-3xl mx-auto leading-relaxed font-medium']"
          data-aos="fade-up" data-aos-delay="200">
          {{ description }}
        </p>

        <!-- Decorative Line -->
        <div :class="['mt-6 flex justify-center', lineAnim]" data-aos="fade-up" data-aos-delay="300">
          <div :class="['h-1 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 rounded-full', lineWidth]"></div>
        </div>
      </div>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const containerPadding = computed(() => (isCompact.value ? 'py-8' : isRegular.value ? 'py-12' : 'py-20'))
  const titleSize = computed(() => (isCompact.value ? 'text-3xl md:text-5xl' : isRegular.value ? 'text-4xl md:text-6xl' : 'text-5xl md:text-7xl'))
  const descSize = computed(() => (isCompact.value ? 'text-base md:text-lg' : isRegular.value ? 'text-lg md:text-xl' : 'text-xl md:text-2xl'))
