<template>
	<div class="relative">
		...
		<input ref="inputRef" :value="modelValue" @input="onInput" @keydown.enter.prevent="$emit('enter')"
			@keydown.escape="$emit('escape')" :placeholder="placeholder"
			class="w-full pl-10 pr-10 py-3 text-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:text-white placeholder:text-zinc-400 transition-colors" />
