    ...
    <Teleport to="body">
      <div v-if="isOpen" class="fixed inset-0 z-50 px-4 py-6 md:py-12">
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-labelledby="isOpen ? 'search-modal-title' : undefined"
        ...
        >
          ...
          <div class="w-full flex flex-col">
              ...
              <div class="mx-3 md:mx-4">
                <div class="relative">
                  <SearchInput
                    ref="searchInput"
                    v-model="searchQuery"
                    placeholder="Search posts, authors, categories and tags"
                    @enter="performSearchImmediate"
                    @clear="() => (searchQuery = '')"
                    class="w-full"
                  />
            ...
            <!-- Results area -->
            <div
              class="p-4 md:p-6 flex-1 min-h-0 custom-scroll bg-transparent backdrop-blur-sm md:backdrop-blur-md"
            >
              <!-- allow touch scrolling within the modal and prevent overscroll propagation -->
              <div
                ref="resultsWrapperRef"
                class="p-0 flex-1 overflow-y-auto custom-scroll min-h-0"
  ...
  try {
    ...
    if (fullSearchController.value) {
      try {
        ...
        console.error('Error aborting previous full search:', e)
      }
    }
    fullSearchController.value = new AbortController()
    const response: SearchResultsType = await searchService.search(
      searchQuery.value,
      20,
      signal || fullSearchController.value.signal,
    )
...
const performSuggest = debounce(async () => {
  ...
  try {
    ...
    console.error('Error aborting previous suggest request:', e)
  }
  ...
  try {
    ...
    try {
      ...
      console.error('Error aborting previous quick request:', e)
    }
    ...
    try {
      ...
      previewResults.value = rows.map((r: any) => {
        ...
        if (r.title) {
          type = 'post'
          url = r.slug ? `/blog/${r.slug}` : '/blog'
        } else if (r.name) {
          // Authors have 'name' instead of 'title'
          type = 'author'
          url = r.slug ? `/authors/${r.slug}` : '/authors'
        }
        ...
        return {
          ...
          highlightedExcerpt: highlightText(
            r.excerpt || r.description || r.bio || '',
            searchQuery.value,
          ),
        }
      })
    } catch (e) {
      console.error('Error fetching preview results:', e)
      previewResults.value = []
    }
...
const selectSuggestion = async (
  payload: string | { text: string; type: string; url?: string; slug?: string },
) => {
...
const handleKeydown = (event: KeyboardEvent) => {
  ...
  switch (event.key) {
    ...
    case 'Enter': {
      ...
      if (mode.value === 'mixed') {
        ...
        selected =
          ...
          suggestionItems.value[selectedIndex.value] ||
    ...
    case '2': {
      if (event.altKey) {
        event.preventDefault()
        if (groupStarts.value['author'] !== undefined)
          selectedIndex.value = groupStarts.value['author']
      }
      break
    }
    case '3': {
      if (event.altKey) {
        event.preventDefault()
        if (groupStarts.value['category'] !== undefined)
          selectedIndex.value = groupStarts.value['category']
      }
      break
    }
    case '4': {
...
watch(isOpen, async (newValue: any) => {
  if (newValue) {
    await nextTick()
    ...
    try {
      ...
      console.error('Error focusing search input on mount:', e)
    }
  }
})
  ...
  try {
    if (open) {
      ...
      if (!document.getElementById(SCROLL_LOCK_STYLE_ID)) {
        ...
        s.textContent =
          'body.search-modal-open { overflow: hidden !important; overscroll-behavior: contain !important; }'
    ...
    console.error('Error removing scroll lock style on toggle:', e)
  }
})
...
onUnmounted(() => {
  try {
    ...
    console.error('Error restoring body scroll on unmount:', e)
  }
})
  ...
  nextTick(() => {
    ...
    ;(el as any).__searchModalCleanup = () => {
      try {
        ...
        console.error('Error removing wheel listener:', e)
      }
      try {
        el.removeEventListener('touchstart', onTouchStart)
      } catch (e) {
        console.error('Error removing touchstart listener:', e)
      }
      try {
        el.removeEventListener('touchmove', onTouchMove)
      } catch (e) {
        console.error('Error removing touchmove listener:', e)
      }
    }
  })
})
...
onUnmounted(() => {
  ...
  try {
    ...
    console.error('Error aborting suggest controller on unmount:', e)
  }
  try {
    quickController.value?.abort()
  } catch (e) {
    console.error('Error aborting quick controller on unmount:', e)
  }
  try {
    fullSearchController.value?.abort()
  } catch (e) {
    console.error('Error aborting full search controller on unmount:', e)
  }
  ...
  try {
    ...
    console.error('Error during results wrapper cleanup on unmount:', e)
  }
})
