<template>
  <div class="flex items-center space-x-2">
    <span v-if="label" class="text-sm font-medium text-gray-700 dark:text-gray-300 mr-2">
      {{ label }}
    </span>

    <!-- Share Buttons -->
    <div class="flex items-center space-x-2">
      <!-- Twitter/X -->
      <button
        @click="shareOnTwitter"
        class="p-2 text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-400 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        :title="`Share on ${twitterLabel}`"
        :aria-label="`Share on ${twitterLabel}`"
      >
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path
            d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"
          />
        </svg>
      </button>

      <!-- Facebook -->
      <button
        @click="shareOnFacebook"
        class="p-2 text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        title="Share on Facebook"
        aria-label="Share on Facebook"
      >
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path
            d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"
          />
        </svg>
      </button>

      <!-- LinkedIn -->
      <button
        @click="shareOnLinkedIn"
        class="p-2 text-gray-500 hover:text-blue-700 dark:text-gray-400 dark:hover:text-blue-600 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        title="Share on LinkedIn"
        aria-label="Share on LinkedIn"
      >
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path
            d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"
          />
        </svg>
      </button>

      <!-- WhatsApp -->
      <button
        v-if="showWhatsApp"
        @click="shareOnWhatsApp"
        class="p-2 text-gray-500 hover:text-green-600 dark:text-gray-400 dark:hover:text-green-500 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        title="Share on WhatsApp"
        aria-label="Share on WhatsApp"
      >
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          <path
            d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.864 3.488"
          />
        </svg>
      </button>

      <!-- Copy Link -->
      <button
        @click="copyLink"
        class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        :title="copied ? 'Copied!' : 'Copy link'"
        :aria-label="copied ? 'Link copied' : 'Copy link'"
      >
        <CheckIcon v-if="copied" class="w-5 h-5 text-green-500" />
        <LinkIcon v-else class="w-5 h-5" />
      </button>

      <!-- Email -->
      <button
        v-if="showEmail"
        @click="shareViaEmail"
        class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        title="Share via email"
        aria-label="Share via email"
      >
        <EnvelopeIcon class="w-5 h-5" />
      </button>

      <!-- Print -->
      <button
        v-if="showPrint"
        @click="printPage"
        class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
        title="Print page"
        aria-label="Print page"
      >
        <PrinterIcon class="w-5 h-5" />
      </button>
    </div>

    <!-- Share Count (optional) -->
    <div
      v-if="showShareCount && shareCount > 0"
      class="text-sm text-gray-500 dark:text-gray-400 ml-2"
    >
      {{ shareCount }} shares
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, computed } from 'vue'
import { LinkIcon, CheckIcon, EnvelopeIcon, PrinterIcon } from '@heroicons/vue/24/outline'

interface Props {
  url?: string
  title?: string
  description?: string
  image?: string
  hashtags?: string[]
  via?: string
  label?: string
  showWhatsApp?: boolean
  showEmail?: boolean
  showPrint?: boolean
  showShareCount?: boolean
  shareCount?: number
  twitterLabel?: string
}

const props = withDefaults(defineProps<Props>(), {
  url: '',
  title: '',
  description: '',
  image: '',
  hashtags: () => [],
  via: '',
  label: '',
  showWhatsApp: true,
  showEmail: true,
  showPrint: false,
  showShareCount: false,
  shareCount: 0,
  twitterLabel: 'X',
})

// Reactive state
const copied = ref(false)

// Computed properties
const shareUrl = computed(() => props.url || window.location.href)
const shareTitle = computed(() => props.title || document.title)
const shareDescription = computed(() => props.description || '')

// Share functions
const shareOnTwitter = () => {
  const text = shareTitle.value
  const hashtags = props.hashtags.join(',')
  const via = props.via

  let url = `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl.value)}&text=${encodeURIComponent(text)}`

  if (hashtags) {
    url += `&hashtags=${encodeURIComponent(hashtags)}`
  }

  if (via) {
    url += `&via=${encodeURIComponent(via)}`
  }

  openPopup(url, 'twitter')
}

const shareOnFacebook = () => {
  const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl.value)}`
  openPopup(url, 'facebook')
}

const shareOnLinkedIn = () => {
  let url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl.value)}`

  if (shareTitle.value) {
    url += `&title=${encodeURIComponent(shareTitle.value)}`
  }

  if (shareDescription.value) {
    url += `&summary=${encodeURIComponent(shareDescription.value)}`
  }

  openPopup(url, 'linkedin')
}

const shareOnWhatsApp = () => {
  const text = `${shareTitle.value} ${shareUrl.value}`
  const url = `https://wa.me/?text=${encodeURIComponent(text)}`

  if (isMobile()) {
    window.location.href = url
  } else {
    openPopup(url, 'whatsapp')
  }
}

const copyLink = async () => {
  try {
    if (navigator.clipboard && window.isSecureContext) {
      await navigator.clipboard.writeText(shareUrl.value)
    } else {
      // Fallback for older browsers
      const textArea = document.createElement('textarea')
      textArea.value = shareUrl.value
      textArea.style.position = 'fixed'
      textArea.style.left = '-999999px'
      textArea.style.top = '-999999px'
      document.body.appendChild(textArea)
      textArea.focus()
      textArea.select()
      document.execCommand('copy')
      textArea.remove()
    }

    copied.value = true
    setTimeout(() => {
      copied.value = false
    }, 2000)

    // Optional: emit event for analytics
    emitShareEvent('copy')
  } catch (error) {
    console.error('Failed to copy:', error)
  }
}

const shareViaEmail = () => {
  const subject = encodeURIComponent(shareTitle.value)
  const body = encodeURIComponent(`${shareDescription.value}\n\n${shareUrl.value}`)
  const url = `mailto:?subject=${subject}&body=${body}`
  window.location.href = url
  emitShareEvent('email')
}

const printPage = () => {
  window.print()
  emitShareEvent('print')
}

// Helper functions
const openPopup = (url: string, platform: string) => {
  const popup = window.open(
    url,
    `share-${platform}`,
    'width=600,height=400,scrollbars=yes,resizable=yes',
  )

  if (popup) {
    popup.focus()
    emitShareEvent(platform)
  }
}

const isMobile = (): boolean => {
  return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)
}

const emitShareEvent = (platform: string) => {
  // Emit custom event for analytics tracking
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('social-share', {
        detail: {
          platform,
          url: shareUrl.value,
          title: shareTitle.value,
        },
      }),
    )
  }
}

// Native Web Share API (if available)
const nativeShare = async () => {
  if (navigator.share) {
    try {
      await navigator.share({
        title: shareTitle.value,
        text: shareDescription.value,
        url: shareUrl.value,
      })
      emitShareEvent('native')
    } catch (error) {
      // User cancelled or error occurred
      console.log('Share cancelled or failed:', error)
    }
  }
}

// Expose native share for parent components
defineExpose({
  nativeShare,
  shareOnTwitter,
  shareOnFacebook,
  shareOnLinkedIn,
  shareOnWhatsApp,
  copyLink,
  shareViaEmail,
})
</script>

<style scoped>
/* Additional button hover effects */
@media (prefers-reduced-motion: no-preference) {
  button {
    transition: all 0.2s ease-in-out;
  }

  button:hover {
    transform: translateY(-1px);
  }

  button:active {
    transform: translateY(0);
  }
}

/* Custom animations for copy button */
.copy-success {
  animation: pulse 0.6s ease-in-out;
}

@keyframes pulse {
  0%,
  100% {
    transform: scale(1);
  }

  50% {
    transform: scale(1.05);
  }
}

/* Ensure proper spacing on mobile */
@media (max-width: 640px) {
  .flex.space-x-2 {
    gap: 0.375rem;
  }
}
</style>
