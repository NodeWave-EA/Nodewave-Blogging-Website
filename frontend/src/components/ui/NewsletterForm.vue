<template>
  <form @submit.prevent="handleSubmit" class="newsletter-form space-y-4">
    <!-- Email Input -->
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="flex-1">
        <label for="newsletter-email" class="sr-only">Email address</label>
        <input id="newsletter-email" v-model="form.email" type="email" required placeholder="Enter your email address"
          ...
          :class="{ 'border-red-500 focus:ring-red-500': error }" :disabled="loading" />
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
        ...
        <select v-model="form.preferences!.frequency"
          class="w-full px-3 py-2 text-sm border border-zinc-300/50 dark:border-zinc-600/50 rounded-lg bg-white/80 dark:bg-zinc-700/80 text-zinc-900 dark:text-zinc-100 focus:outline-none">
      ...
      <div>
        <label class="block text-xs text-zinc-500 dark:text-zinc-400 mb-1">Format</label>
        <select v-model="form.preferences!.digest_format"
          class="w-full px-3 py-2 text-sm border border-zinc-300/50 dark:border-zinc-600/50 rounded-lg bg-white/80 dark:bg-zinc-700/80 text-zinc-900 dark:text-zinc-100 focus:outline-none">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <label class="inline-flex items-center gap-2 text-sm">
        <input type="checkbox" v-model="form.preferences!.include_featured" />
        Include featured
      </label>
      <label class="inline-flex items-center gap-2 text-sm">
        <input type="checkbox" v-model="form.preferences!.include_new_posts" />
        Include new posts
      </label>
      <label class="inline-flex items-center gap-2 text-sm">
        <input type="checkbox" v-model="form.preferences!.include_trending" />
        Include trending
      </label>
    </div>
    ...
    <!-- Categories (topics) -->
    <div>
      <label class="block text-xs text-zinc-500 dark:text-zinc-400 mb-1">Categories</label>
      <div class="flex flex-wrap gap-2">
        <button v-for="cat in categories" :key="cat.id" type="button" @click="toggleCategory(cat.id)"
          class="px-3 py-1 rounded-full text-xs border"
          :class="selectedCategories.has(cat.id) ? 'bg-blue-600 text-white border-blue-600' : 'bg-transparent border-zinc-300/50 dark:border-zinc-600/50'">
          {{ cat.name }}
        </button>
      </div>
    </div>
...
<script setup lang="ts">
  import { categoriesApi, newsletterApi } from '@/services/blog';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Only send selected category IDs for categories relation at the top level
      ...
      await newsletterApi.subscribe(payload as any)
