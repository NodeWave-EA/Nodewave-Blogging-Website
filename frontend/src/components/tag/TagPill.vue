<template>
  <router-link
    :to="to || `/tags/${tag.slug}`"
    class="inline-flex items-center px-4 py-2 rounded-full font-semibold text-sm transition-all"
    :class="[
...
const displayCount = computed(() => {
  // Prefer numeric post_count when available; fall back to the length of the
  // server-provided `blog_posts` array (some endpoints return nested posts),
  // otherwise show 0.
  if (typeof tag.post_count === 'number') return tag.post_count
  const bp = (tag as any).blog_posts
