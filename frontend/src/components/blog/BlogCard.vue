<template>
  <article
    class="group relative overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-lg hover:shadow-2xl hover:shadow-blue-500/10 bg-white/80 dark:bg-slate-900/60 backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] max-w-3xl mx-auto"
  >
    <!-- Glow Overlay (non-blocking) -->
    <div
      class="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-transparent to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl pointer-events-none"
    ></div>
    ...
    <div class="relative aspect-[16/10] sm:aspect-[16/9] overflow-hidden">
      ...
      <div v-if="post.featured" class="absolute top-3 left-3 z-20">
        <span
          class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs sm:text-sm font-semibold rounded-full text-white bg-gradient-to-r from-amber-500 to-yellow-500 shadow-lg"
        >
      ...
      <div class="absolute top-3 right-3 z-20">
        <span
          class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs sm:text-sm font-medium bg-white/80 dark:bg-slate-800/70 text-slate-700 dark:text-slate-200 shadow-lg backdrop-blur-sm"
        >
    ...
    <div class="p-6 lg:p-7 relative z-20">
      ...
      <router-link :to="`/blog/${post.slug}`">
        <h3
          class="text-lg sm:text-xl lg:text-2xl font-extrabold text-slate-900 dark:text-white mb-3 line-clamp-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
        >
          {{ post.title }}
        </h3>
      </router-link>
      ...
      <!-- Excerpt -->
      <p
        class="text-sm sm:text-base text-slate-600 dark:text-slate-300 line-clamp-3 mb-5 leading-relaxed"
      >
      ...
      <div v-if="post.tags?.length" class="flex flex-wrap gap-2 mb-5">
        <router-link
          ...
          class="px-2.5 py-1 text-xs font-medium rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition"
        >
          #{{ tag.name }}
        </router-link>
        <span
          v-if="post.tags.length > 3"
          class="px-2.5 py-1 text-xs font-medium rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400"
        >
          +{{ post.tags.length - 3 }}
        </span>
      </div>
      ...
      <!-- Footer -->
      <div
        class="flex items-center justify-between pt-5 border-t border-slate-200 dark:border-slate-700"
      >
        <!-- Author (clickable) -->
        <div v-if="post.author" class="flex items-center gap-3">
          <router-link
            :to="`/author/${post.author.slug || post.author.id}`"
            class="flex items-center gap-3"
          >
            <!-- If author exists but has no avatar, getStrapiImageUrl will return the placeholder avatar URL -->
            <img
              :src="getStrapiImageUrl(post.author?.avatar)!"
        ...
        <div class="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
          ...
          <span v-if="(displayedLikeCount ?? 0) > 0" class="flex items-center gap-1">
            <component
              :is="metricsStore.getMetrics(post.id)?.liked ? HHeartSolid : HHeartIcon"
              class="w-4 h-4"
...
const displayedViewCount = computed(
  () => metricsStore.getMetrics(post.id)?.view_count ?? post.view_count ?? 0,
)
const displayedLikeCount = computed(
  () => metricsStore.getMetrics(post.id)?.like_count ?? post.like_count ?? 0,
)
