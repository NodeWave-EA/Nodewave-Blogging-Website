<template>
	<div class="relative inline-block" @keydown.down.prevent="focusNext" @keydown.up.prevent="focusPrev"
		...
		<transition name="fade">
			<ul v-if="open" role="listbox" tabindex="-1"
				...
				<li v-for="(opt, idx) in optionsToUse" :key="opt.value" :data-index="idx"
					:aria-selected="modelValue === opt.value" role="option" @mousedown.prevent="selectOption(opt.value)"
					@mouseenter="focusIndex(idx)"
					:class="['px-4 py-2 text-sm cursor-pointer truncate', modelValue === opt.value ? 'bg-slate-100 dark:bg-slate-900 font-semibold' : 'hover:bg-slate-50 dark:hover:bg-slate-700']"
