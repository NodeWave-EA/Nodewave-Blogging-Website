              ...
              <transition name="fade">
                ...
                >
                  <li
                    ...
                    role="option"
                    @click.prevent.stop="
                      opt.value === '' ? clearAllCategories() : toggleCategory(opt.value)
                    "
                    :class="[
                      'px-4 py-2 text-sm cursor-pointer truncate flex items-center gap-2',
                      opt.value === ''
                        ? 'font-medium'
                        : isCategorySelected(opt.value)
                  ...
                  >
                    <input
                      ...
                      type="checkbox"
                      class="mr-2"
                      :checked="isCategorySelected(opt.value)"
                      readonly
                    />
                    <span>{{ opt.label }}</span>
                  </li>
...
const hasActiveFilters = computed(() => {
  return !!(
    (props.showSearch && localSearch.value) ||
    (localCategories.value && localCategories.value.length > 0)
  )
})
