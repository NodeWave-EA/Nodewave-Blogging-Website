  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-20" data-aos="fade-up">
        <h2
          class="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight text-zinc-900 dark:text-white"
        >
          Featured Posts
        </h2>
        <p
          class="mt-4 max-w-2xl mx-auto text-base sm:text-lg md:text-xl text-zinc-600 dark:text-zinc-300"
        >
          Hand-picked highlights from our latest writing — prioritized by editorial choice and
          recency.
        </p>
        <div
          class="mt-6 w-20 h-1 mx-auto rounded-full bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-600"
        ></div>
      </div>
...
const CACHE_KEY = 'featured_posts_v1'
...
function readCache() {
  try {
    ...
    if (!isCacheFresh(parsed.ts)) return parsed
    return parsed
  } catch {
    return null
  }
...
function writeCache(data: BlogPost[]) {
  try {
...
const initialize = async () => {
  ...
  if (cache && cache.data) {
    featured.value = (cache.data || []).slice(0, 6)
    loading.value = false
    if (!isCacheFresh(cache.ts)) {
