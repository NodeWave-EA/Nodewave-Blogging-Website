<template>
	<article :class="cardRootClass">
		<!-- HERO / FULLWIDTH -->
		<div v-if="variant === 'hero' || variant === 'fullwidth'" class="relative w-full overflow-hidden">
			<!-- Blurred background -->
			<div v-if="lowRes && imageUrl" :style="{ backgroundImage: `url(${lowRes})` }"
				class="absolute inset-0 bg-center bg-cover blur-3xl scale-105 transition-opacity duration-700"
				:class="{ 'opacity-100': !loaded, 'opacity-0': loaded }"></div>

			<!-- Image -->
			<router-link :to="`/blog/${post.slug}`" class="block">
				<img v-if="imageUrl" :src="imageUrl" :alt="imageAlt || post.title" loading="lazy" @load="onImgLoad"
			...
			<div class="absolute inset-0 flex items-end md:items-center justify-center p-6 md:p-12">
				<div class="text-center max-w-3xl text-white space-y-4">
					...
					<div class="flex flex-wrap items-center justify-center gap-2 mb-3">
						<router-link v-for="cat in post.categories?.slice(0, 2) || []" :key="cat.id" :to="`/category/${cat.slug}`"
					...
					<router-link :to="`/blog/${post.slug}`">
						<h3 class="text-2xl sm:text-3xl md:text-5xl font-extrabold tracking-tight drop-shadow-lg hover:underline">
							{{ post.title }}
						</h3>
					</router-link>
		...
		<div v-else
			...
			<router-link :to="`/blog/${post.slug}`" class="relative aspect-[16/9] overflow-hidden block">
				...
				<img v-if="imageUrl" :src="imageUrl" :alt="imageAlt || post.title" loading="lazy" @load="onImgLoad"
					...
					:class="{ 'opacity-0': !loaded, 'opacity-100': loaded }" />
				<div v-else class="flex items-center justify-center w-full h-full bg-zinc-100 dark:bg-zinc-800">
			...
			<div class="p-5 flex flex-col flex-grow">
				...
				<div class="flex items-center justify-between mb-3">
					<div class="flex items-center gap-2">
						<router-link v-for="category in post.categories?.slice(0, 1) || []" :key="category.id"
				...
				<div v-if="post.tags?.length" class="mt-3 flex flex-wrap gap-2">
					<router-link v-for="tag in post.tags.slice(0, 3)" :key="tag.id" :to="`/tag/${tag.slug}`"
				...
				<!-- Footer -->
				<div class="pt-5 mt-auto flex items-center justify-between border-t border-zinc-200/50 dark:border-zinc-700/50">
					<!-- Author -->
					<router-link v-if="post.author" :to="`/author/${post.author.slug}`"
						class="text-sm text-zinc-600 dark:text-zinc-400 flex items-center gap-3 mt-3 hover:text-blue-600">
						<img v-if="post.author?.avatar && imageAuthUrl" :src="imageAuthUrl" :alt="post.author?.name"
...
<script lang="ts" setup>
	...
	const lowRes = computed(() => {
		...
		if (!i) return null
		const candidate = (i.formats && (i.formats.small?.url || i.formats.thumbnail?.url)) || i.previewUrl || i.url
