    ...
    <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
      <div class="flex-shrink-0">
        <img
          :src="avatarUrl"
          :alt="author.name"
          class="w-36 h-36 rounded-full object-cover ring-4"
        />
      </div>

      <div class="flex-1">
        <h1 class="text-3xl md:text-4xl font-extrabold leading-tight text-black dark:text-white">
          {{ author.name }}
        </h1>
        <p
        ...
        <div
          class="mt-4 flex flex-wrap items-center gap-4 text-sm text-zinc-700 dark:text-zinc-300"
        >
          ...
          <div v-if="postsCount > 0" class="flex items-center gap-2">
            ...
            <span class="font-medium"
              >{{ postsCount }} {{ postsCount === 1 ? 'post' : 'posts' }}</span
            >
          </div>
        </div>
        ...
        <div class="mt-4 flex items-center gap-3">
          <SocialLink
            v-for="(link, idx) in author.social_links || []"
            :key="link.url ?? link.platform ?? idx"
            :link="link"
            size="w-5 h-5"
          />
        </div>
