<template>
  ...
  >
    ...
    <div class="text-center mb-6 relative">
      <div class="inline-block">
        <router-link :to="authorLink" class="group inline-block">
          <img
            ...
            :alt="author.name"
            :class="[
              'w-24 h-24 rounded-full object-cover shadow-lg transition-all duration-300',
              avatarRingClass,
            ]"
          />
        </router-link>
      ...
      <span v-if="postsCount > 0" :class="postBadgeClass"
        >{{ postsCount }}
    ...
    <div class="text-center">
      ...
      <!-- Social Links -->
      <div
        v-if="author.social_links && author.social_links.length"
      ...
      >
        ...
        <span
          ...
          class="px-2 py-1 text-xs font-medium rounded-full bg-transparent text-black dark:text-white border border-black dark:border-white"
          >+{{ author.expertise_areas.length - 3 }} more</span
        >
      </div>
    </div>
  </div>
