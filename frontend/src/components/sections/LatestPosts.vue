    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        <article
          ...
          class="group bg-white/80 dark:bg-zinc-900/80 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 backdrop-blur-sm"
        >
            ...
            <div class="p-6 flex flex-col h-full">
              ...
              <div v-if="post.tags && post.tags.length > 0" class="flex flex-wrap gap-2 mb-4">
                <span
                  ...
                  :key="tag.id"
                  class="px-2 py-1 text-xs font-medium bg-zinc-100 dark:bg-zinc-800/50 text-zinc-600 dark:text-zinc-300 rounded"
                >
                  #{{ tag.name }}
                </span>
                <span
              ...
              <!-- Author & Reading Time -->
              <div
                class="flex items-center justify-between pt-4 border-t border-zinc-200/50 dark:border-zinc-800/50"
              >
      ...
      >
        <router-link
          to="/blog"
          class="group relative inline-flex items-center px-8 py-3 text-base font-medium rounded-full text-blue-600 bg-white/80 hover:bg-white/90 dark:bg-zinc-900/80 dark:text-blue-400 dark:hover:bg-zinc-800/90 backdrop-blur-sm transition-all duration-300 shadow-lg hover:shadow-xl"
        >
          View All Posts
          <ArrowRightIcon class="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
          <div
            class="absolute inset-0 rounded-full bg-gradient-to-r from-blue-600/10 to-purple-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          ></div>
        </router-link>
      </div>
    </div>
...
import { BookOpenIcon, UserCircleIcon, ClockIcon, ArrowRightIcon } from '@heroicons/vue/24/outline'
...
const loadLatestPosts = async () => {
  try {
    loading.value = true
    const response = await blogService.getAll(1, 6)
