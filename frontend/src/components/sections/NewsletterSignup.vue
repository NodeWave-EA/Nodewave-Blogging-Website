<template>
  <section id="newsletter" class="py-16">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div
        class="rounded-2xl border border-border bg-transparent backdrop-blur-md shadow-lg p-8 sm:p-10 text-center"
        data-aos="fade-up"
      >
        <!-- Icon -->
        <div
        ...
        <div class="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="text-center">
            ...
            <p class="text-sm text-muted-foreground">
              Access subscriber-only articles and previews
            </p>
          </div>
...
const loadNewsletterStats = async () => {
  try {
    const response: any = await apiService.get(
      '/newsletters?pagination[page]=1&pagination[pageSize]=1',
    )
