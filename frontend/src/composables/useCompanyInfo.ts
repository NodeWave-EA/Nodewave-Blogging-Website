...
export function useCompanyInfo() {
  const fetchCompanyInfo = async (): Promise<void> => {
    ...
    try {
      ...
      // apiService.get returns the parsed response.data, which for this route is { data: CompanyInfo }
      // Request populated relations (media, seo, social links)
      const response = await apiService.get<{ data: CompanyInfo }>("/company-info?populate=*")
