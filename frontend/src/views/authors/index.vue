    ...
    <div v-else class="py-8">
      ...
      <div class="container mx-auto px-4 max-w-6xl">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <router-link v-for="author in authors" :key="author.id" :to="`/authors/${author.slug || author.id}`"
            ...
            <div class="text-center mb-6">
              <div class="relative inline-block">
                <img v-if="getStrapiImageUrl(author.profile_picture)" :src="getStrapiImageUrl(author.profile_picture)!"
                  :alt="author.name"
                ...
                <UserCircleIcon v-else
                  class="w-24 h-24 text-zinc-400 group-hover:text-blue-500 transition-colors duration-300" />

                <!-- Online status indicator (if available) -->
                <div v-if="author.is_active"
            ...
            <div class="text-center">
              <h3 class="text-xl font-bold text-black dark:text-white mb-2 group-hover:underline transition-colors">
                {{ author.name }}
              </h3>

              <p v-if="author.role" class="text-blue-600 dark:text-blue-400 font-medium mb-3">
                {{ author.role }}
              </p>
              ...
              <!-- Stats -->
              <div class="flex items-center justify-center gap-4 text-sm text-black dark:text-white mb-4">
                <div class="flex items-center gap-1">
                  <DocumentTextIcon class="w-4 h-4" />
                  <span>{{ author.post_count || 0 }} posts</span>
                </div>
                <div v-if="author.follower_count" class="flex items-center gap-1">
              ...
              <!-- Specialties/Tags -->
              <div v-if="author.specialties && author.specialties.length > 0"
                class="flex flex-wrap justify-center gap-2">
                <span v-for="specialty in author.specialties.slice(0, 3)" :key="specialty"
                ...
                <span v-if="author.specialties.length > 3"
                  class="px-2 py-1 text-xs font-medium rounded-full bg-transparent text-black dark:text-white border border-black dark:border-white">
      ...
      <div v-if="featuredAuthors.length > 0" class="container mx-auto px-4 max-w-6xl mt-16">
        ...
        <div class="flex flex-wrap justify-center gap-6">
          <router-link v-for="author in featuredAuthors" :key="author.id" :to="`/authors/${author.slug || author.id}`"
            ...
            <img v-if="getStrapiImageUrl(author.profile_picture)" :src="getStrapiImageUrl(author.profile_picture)!"
              :alt="author.name" class="w-12 h-12 rounded-full object-cover" />
...
<script setup lang="ts">
  ...
  const featuredAuthors = computed(() => {
    return authors.value
      .filter((author) => (author.post_count || 0) > 0)
