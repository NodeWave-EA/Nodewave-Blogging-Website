      ...
      <header class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl py-16 mb-12">
        <div class="container mx-auto px-4 max-w-4xl">
          ...
          <nav class="flex items-center space-x-2 text-sm text-zinc-500 dark:text-zinc-400 mb-8">
            ...
            <router-link to="/blog" class="hover:text-blue-600 dark:hover:text-blue-400"
              >Blog</router-link
            >
            <ChevronRightIcon class="w-4 h-4" />
          ...
          <!-- Title -->
          <h1
            class="text-4xl md:text-5xl font-bold text-zinc-900 dark:text-white mb-6 leading-tight"
          >
            {{ post.title }}
          </h1>
          ...
          <p
            v-if="post.excerpt"
            class="text-xl text-zinc-600 dark:text-zinc-300 mb-8 leading-relaxed"
          >
            {{ post.excerpt }}
          </p>
          ...
          <div class="flex flex-wrap items-center gap-6 text-zinc-500 dark:text-zinc-400">
            ...
            <div v-if="post.author" class="flex items-center gap-3">
              ...
              />
              <UserCircleIcon v-else class="w-12 h-12 text-zinc-400" />
              <div>
                <router-link
                  :to="`/authors/${post.author.slug || post.author.id}`"
                  class="text-zinc-900 dark:text-white font-semibold hover:text-blue-600 dark:hover:text-blue-400"
                >
                  {{ post.author.name }}
                </router-link>
                <p class="text-sm">{{ post.author.role || 'Author' }}</p>
              </div>
            </div>
      ...
      <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div v-if="post.author" class="mb-12 p-8 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl">
          ...
          <div class="flex items-start gap-6">
            ...
            />
            <UserCircleIcon v-else class="w-20 h-20 text-zinc-400" />
            <div class="flex-1">
              <router-link
                :to="`/authors/${post.author.slug || post.author.id}`"
              ...
              <p v-if="post.author.role" class="text-zinc-600 dark:text-zinc-300 text-sm mb-3">
                {{ post.author.role }}
              </p>
              ...
              <div v-if="post.author.social_links" class="flex items-center gap-3">
                <a
                  v-if="post.author.social_links.twitter"
                ...
                <a
                  v-if="post.author.social_links.github"
                ...
                <a
                  v-if="post.author.social_links.website"
        ...
        <!-- Navigation -->
        <div
          class="flex justify-between items-center py-8 border-t border-zinc-200 dark:border-zinc-700"
        >
          <router-link
            to="/blog"
...
const shareUrls = computed(() => {
  ...
  const description = encodeURIComponent(post.value.excerpt || post.value.title)
...
const fetchPost = async (slug: string) => {
  try {
    ...
    if (post.value) {
      await blogPostsApi.incrementViewCount(post.value.id).catch(() => {
